package de.th.wildau.demo.jbpm.simulation.util;
 ...
 * 
 * {@link https
 * ://github.com/droolsjbpm/droolsjbpm-integration/tree/r6.0.1.Final/
 * jbpm-simulation/src/test/java/org/jbpm/simulation/helper/TestUtils.java}
 *
 */
...
public class TestUtils {
				...
				if (expected != null && elements.size() == expected.size()) {
					...
					for (FlowElement fe : elements) {
						if (!expected.contains(fe.getId())) {
							System.err
									.println("Following element not matched: "
											+ fe.getId() + " " + fe.getName());
							match = false;
							break;
						}
	...
	public static void printOutPaths(List<PathContext> paths,
			JSONObject jsonPaths, String name) {
		if (!"true".equalsIgnoreCase(System.getProperty("test.debug.off"))) {
			System.out.println("###################" + name
					+ "###################");
			for (PathContext context : paths) {
	...
	public static StatefulKnowledgeSession createSession(String process) {
		...
		builder.add(ResourceFactory.newClassPathResource(process),
				ResourceType.BPMN2);

		KnowledgeBase kbase = builder.newKnowledgeBase();
		KieSessionConfiguration config = KnowledgeBaseFactory
				.newKnowledgeSessionConfiguration();
		config.setOption(ClockTypeOption.get("pseudo"));
		StatefulKnowledgeSession session = kbase.newStatefulKnowledgeSession(
				config, EnvironmentFactory.newEnvironment());
		session.getEnvironment().set("NodeInstanceFactoryRegistry",
				SimulationNodeInstanceFactoryRegistry.getInstance());
	...
	public static List<FlowElement> removeDuplicates(Set<FlowElement> orig) {
		...
		System.out.println("Size of flow elements after removing duplicates "
				+ unique.size());
		return unique;
	}

	public static ReleaseId createKJarWithMultipleResources(String id,
			String[] resourceFiles, ResourceType[] types) throws IOException {
		...
		for (int i = 0; i < resourceFiles.length; i++) {
			Resource resource = ResourceFactory
					.newClassPathResource(resourceFiles[i]);
